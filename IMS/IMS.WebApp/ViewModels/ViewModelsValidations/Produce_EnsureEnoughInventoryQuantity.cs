            ...
            if (produceViewModel != null)
            {
                if (produceViewModel.Product != null && produceViewModel.Product.ProductInventories != null)
                {
                    foreach (var pi in produceViewModel.Product.ProductInventories)
                    {
                        ...
                        {
                            {
                                return new ValidationResult($"The Inventory ({pi.Inventory.InventoryName}) is not enough tp produce {produceViewModel.QuantityToProduce} products.", new[] { validationContext.MemberName });
                            }
                        }
                    }
                }
